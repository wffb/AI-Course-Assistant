...
namespace new_test{
    ...
    class Gpt{
        ...
        static async Task Main(string[] args){
            ...
            // await My_assistant.CreateAssistant(GetService());
            // await My_assistant.AllTest(GetService());

            LLM.Init(GetService());
            await LLM.GetAsw("hello");
...
//这里定义一个异步方法，因为原库的openAIService.ChatCompletion.CreateCompletion()是一个异步方法，我在原库的GitHub社区看到有人建议在方法名后面加一个async强调异步，看来不少人中招了，把它当同步方法用，哈哈哈。
    ...
    public static OpenAIService GetService()
    {
        ...
        {
            ApiKey = "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",
            UseBeta = true
        };
