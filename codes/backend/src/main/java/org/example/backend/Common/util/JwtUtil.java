...
//             username= (String) claims.get("sub");
     ...
     * 从token中获取管理员标识
     */
    public Boolean getIsTeacherFromToken(String token) {
        Boolean isAdmin;
        try {
            Claims claims = getClaimsFromToken(token);
            isAdmin =  (Boolean)claims.get("isTeacher");
        } catch (Exception e) {
            isAdmin = null;
        }
        return isAdmin;
...
//    }
    ...
    public String generateToken(UserDetails userDetails,boolean isTeacher) {
        Map<String, Object> claims = new HashMap<>();
